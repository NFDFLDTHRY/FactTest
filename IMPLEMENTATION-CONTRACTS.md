...
D18 ANNOTATION (implementation reality relevant to these contracts; D17).
- WebGPU on Linux Chromium 141: the WebGPU service is disabled by default; FactTest's admitted WebGPU path runs under the unsafe
  switch set on SwiftShader, a CPU (software) fallback adapter; WGSL is unexercised, so the WebGPU family is [GAP] against its
  admission contract (graph Q19, CAP-WEBGPU).  No hardware GPU evidence exists ([UNK]).
- Storage durability: persist() cannot succeed in the headless shell (every permission ASK); full Chrome grants durable storage
  to installed or important sites; the durability of an installed Factory is unobserved ([UNK]).
- Kernel identity: install-path independent over the executable sections (proof-sets.json exec_identity); only LLVM's
  promoted-symbol names in the custom "name" section carry the rustup install name.
