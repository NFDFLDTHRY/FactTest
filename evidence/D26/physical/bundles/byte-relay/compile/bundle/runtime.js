...
import { STRATEGY, STRATEGY_SHA256, select } from './selector.js';
...
function activate() {
  ...
  state.activation = { epoch_id: epochId(), strategy_id: sel.strategy_id, plan_id: sel.plan_id, admission_refs: Object.values(state.admissions).map(r => r.receipt_id), metric_evidence_refs: STRATEGY.goals.map(g => 'metric:' + g.metric), guard_result: sel.guard_results, status: sel.status };
...
export async function init() {
  // the tape names the bundle it came from by the identity of the verified strategy data it executes under
  if (state.epochIndex === -1) island('bundle strategy_data="' + STRATEGY_SHA256 + '" strategy=' + STRATEGY.strategy_id + ' certificate=' + STRATEGY.strategy_certificate_id);
...
/** The requirement of the active plan for `relation` (the verified edge: backends, adapters, conversions). */
function requirementFor(plan, relation) { return (plan.requirements || []).find(r => r.relation === relation) || null; }

/** Execute one authored transfer relation through the verified edge of the active plan (no fixed relation, no fixed backend). */
export async function transfer(relation, bytes) {
  /*EVIDENCE-HOOK:executed*/
  await lossChain;
  const declared = STRATEGY.transfers.find(t => t.relation === relation);
  if (!declared) { record('EXECUTE', relation, { status: 'UNKNOWN_RELATION' }, null); return { status: 'UNKNOWN_RELATION', relation }; }
  if (!state.activePlan) { record('EXECUTE', relation, { status: 'NO_ACTIVE_PLAN' }, null); island('no_active_plan ' + epochId()); return { status: 'NO_ACTIVE_PLAN', relation }; }
  const plan = state.activePlan;
  const req = requirementFor(plan, relation);
  const backend = req && req.backends.length ? req.backends[0] : null;
  const a = backend ? adapterFor(backend) : null;
  const inputSha = await sha256hex(bytes);
  if (!a) {
    record('EXECUTE', relation, { status: 'FAILED', result: Result.UNAVAILABLE, detail: 'the active plan realizes this relation with no emitted adapter', input_sha256: inputSha }, null);
    island('executed ' + epochId() + ' plan=' + plan.plan_id + ' relation=' + relation + ' status=' + Result.UNAVAILABLE);
    return { status: 'FAILED', result: Result.UNAVAILABLE, relation };
  }
  const out = await a.operate(bytes);
  if (out.result !== Result.OK) {
    record('EXECUTE', relation, { status: 'FAILED', result: out.result, detail: out.detail, backend, input_sha256: inputSha }, null);
    island('executed ' + epochId() + ' plan=' + plan.plan_id + ' relation=' + relation + ' status=' + out.result);
    return { status: 'FAILED', result: out.result, detail: out.detail, relation, backend };
  }
  const outputSha = await sha256hex(out.bytes);
  const exact = bytesEqual(out.bytes, bytes);
  record('EXECUTE', relation, { status: 'EXECUTED', plan_id: plan.plan_id, backend, conversions: req.conversions, bytes: bytes.length, input_sha256: inputSha, output_sha256: outputSha, exact }, { output_sha256: inputSha });
  island('executed ' + epochId() + ' plan=' + plan.plan_id + ' relation=' + relation + ' bytes=' + bytes.length + ' input="' + inputSha + '" output="' + outputSha + '" exact=' + exact);
  return { status: 'EXECUTED', plan_id: plan.plan_id, relation, backend, conversions: req.conversions, bytes: out.bytes, exact, input_sha256: inputSha, output_sha256: outputSha };
}
...
window.factRuntime = { init, transfer, destroyBackend, snapshot, STRATEGY };
