...
const eM = ev(relpath(o.manifest), `${M.tracked_files} tracked files at ${M.rev.slice(0, 9)} in ${M.components.length} components (${Object.entries(M.files_by_tier).map(([k, v]) => `${k} ${v}`).join(', ')}); ${nf} finding items`, true);
const eG = ev(relpath(o.gate), `${G.status}: ${G.checks.map(c => `${c.check} ${c.status}`).join(', ')}`, G.status === 'PASS');
const eI = ev(relpath(o.issues), `${I.issues.length} issues: ${Object.entries(byClass).sort().map(([k, v]) => `${k} ${v}`).join(', ')}; by task ${Object.entries(byTask).sort().map(([k, v]) => `${k} ${v}`).join(', ')}`, true);
...
N({ id: FS, class: 'COMPUTATIONAL_FACT', fact_id: FS, subject: `current executable FactTest at ${M.rev.slice(0, 9)}`, predicate: `${M.tracked_files} tracked files tiered (${Object.entries(M.files_by_tier).map(([k, v]) => `${k} ${v}`).join(', ')}); ${live.length} live components (${live.filter(c => c.tier === 'PRODUCTION').length} production, ${live.filter(c => c.tier === 'FACTORY').length} factory, ${live.filter(c => c.tier === 'TEST').length} test), each with owner, consumer and station; required coverage complete; ${F.unassigned_paths.length} unassigned paths; ${F.live_components_without_governing_claim.length} live components had no governing claim before this epoch (now carried by this fact)`,
  required_environment: ['the canonical tree at the recorded head'], constraint_refs: [], status: G.status === 'PASS' ? 'OBS' : 'ERR', note: 'an enumeration, not an execution claim: D22-D26 execute the components', source_ref: relpath(o.manifest), owner: `${E} (derived from the evidence named by its edges)` });
N({ id: FI, class: 'COMPUTATIONAL_FACT', fact_id: FI, subject: `${E} issue inventory`, predicate: `${nf} automated finding items across ${Object.keys(F).length} finding kinds classified into ${I.issues.length} issues: ${Object.entries(byClass).sort().map(([k, v]) => `${k} ${v}`).join(', ')}; scheduled ${Object.entries(byTask).sort().map(([k, v]) => `${k} ${v}`).join(', ')}`,
  required_environment: [], constraint_refs: [], status: G.status === 'PASS' ? 'OBS' : 'ERR', note: 'classification precedes repair (GAP CLASSIFICATION LAW); each issue names its task and surfaces', source_ref: relpath(o.issues), owner: `${E} (derived from the evidence named by its edges)` });
