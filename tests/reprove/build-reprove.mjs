...
N({ id: IMPL, class: 'IMPLEMENTATION', impl_id: IMPL, repo_path: 'tests/reprove/{identity.mjs,select.mjs,run-selected.mjs,lineage.mjs,build-reprove.mjs,gate.mjs,dimensions.json,runbook.json,obligations.json}', commit: `introduced by ${o.delta} (integration commit in LEDGER AFTER)`, kind: 'harness', note: 'environment identity capture, graph-driven minimum affected set, selected re-proof runner', owner: 'FactTest repository path tests/reprove/' });
N({ id: PID, class: 'PROBE', probe_id: PID, proves_fact: [FID, FEXE], command_or_operation: 'node tests/reprove/identity.mjs --graph <graph> --out <dir>: host identity, toolchains by rustup name, repository, browser identity (default + GPU flags, executable launched), authority source tips',
...
N({ id: PSEL, class: 'PROBE', probe_id: PSEL, proves_fact: [FSET], command_or_operation: 'node tests/reprove/select.mjs (graph + identity + git) -> selection; node tests/reprove/run-selected.mjs per kind -> results',
