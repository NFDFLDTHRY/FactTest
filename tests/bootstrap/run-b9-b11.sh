...
cargo +nightly build --release -p factc-wasm-abi -Z build-std=core --target wasm64-unknown-unknown >> "$OUT/b9-build.log" 2>&1
...
node host/harness/kernel-host.mjs "$OUT/factc_wasm_abi.wasm" tests/bootstrap/b7-opaque.ascii --browser > "$OUT/b10-b11-chromium.json"
grep -q '"abi_version": 1' "$OUT/b10-b11-chromium.json"
grep -q '"imports": \[\]' "$OUT/b10-b11-chromium.json"
grep -q 'LANGUAGE_KERNEL_NOT_IMPLEMENTED' "$OUT/b10-b11-chromium.json"
