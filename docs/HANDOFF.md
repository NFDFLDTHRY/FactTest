...
last change        MERGE RECONCILIATION (PR #5 x PR #6): main 1205fda (D14-TECHNICAL-REFERENCE-RESCAN, a parallel line)
                   merged with D19-REPROVE-REOBSERVE 780aee1; both lines kept; the rescan line relocated under its full
                   delta id (design/materialization/D14-MERGE-RECONCILIATION.md)
...
materialized       Factory plane (factory/), no_std compiler kernel (compiler/), host driver + browser harnesses (host/),
                   ...
                   re-proved physically; current evidence epoch; entitled-claim surface Q22); D14-TECHNICAL-REFERENCE-
                   RESCAN (parallel line, PR #5: retrieval route under the egress policy via the publishers' GitHub Pages
                   branches, fixtures/reference corpus of 83 reproducibility copies, D11 pins re-checked, 99 map links +
                   64 authorities audited, epoch D14-RESCAN)
...
LIVE RECORD    design/materialization/LEDGER.md; design/environment-map/ (graph.json = merge of the D11 graph and
               epochs/D12.json .. epochs/D19.json and epochs/D14-RESCAN.json; AUTHORITY-REGISTER.md and TRACEABILITY.md
               are generated); fixtures/reference/ (ROUTES.json retrieval route, REGISTER.json copy identities, published/
               and sources/ reproducibility copies: pins, never current authority; re-check with tests/reference/ingress.mjs);
...
commissioning, language, toolchain, envmap, selfhost, hygiene, reference, reconcile, reprove), evidence/<delta>/, design/ (materialization,
...
D      D11 FACT-AUTHORITY-REOPEN-DENIED     narrowed by D14-TECHNICAL-REFERENCE-RESCAN: 30 authorities / 78 map     [UNK]
                                            links now have a Pages-branch copy of the rendering (not observed at
                                            the host); 23 authorities / 20 links on hosts without a Pages branch
                                            (WHATWG, doc.rust-lang.org, git-scm.com, Chromium) stay source-pinned;
                                            23 published hosts still DENIED (evidence/D14-TECHNICAL-REFERENCE-RESCAN/)
OWNER  D14-RESCAN drifted fragments (9)     absent in the current Pages-branch renderings: js-api #internal-storage,  [ERR]
                                            web-api #streaming-module-compilation-and-instantiation (both also
                                            resolved by D18 to #webassembly-storage / #streaming-modules), IndexedDB
                                            #database-concept, ServiceWorker #fetch-event, sensors #extending-the-
                                            permission-api / -permissions-policy-api, deviceorientation #permissions,
                                            ambient-light #ambientlightsensor-interface, webxr #navigator-xr; nearest
                                            ids in the D14 ANNOTATION lines; law delta needed for the seven not in D18
D      D14-RESCAN CON-EM-D14-001            copies are pins, never current authority (PROPOSED constraint)        [OBS]
B      fixtures/reference size              ~51 MB of authority copies (git-compressed far smaller); re-ingest   [GAP] kept
                                            by a new delta when the register's branch heads move
OWNER  merge reconciliation (PR #5 x #6)   two parallel D14 lines reconciled by an owner-directed git merge       [GAP]
                                            outside station execution; the Factory has no merge station (ff-only
                                            integration); design/materialization/D14-MERGE-RECONCILIATION.md
