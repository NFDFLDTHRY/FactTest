...
D22 ANNOTATION (contract amendment, D22-FACTORY-SELF-QUALIFICATION; prospective, historical receipts and verifications unchanged):

- Receipt integrity (sections 5, 6): a receipt counts only as the Factory wrote it.  Station close records, in the Factory-owned workpiece state outside the workpiece, the receipt path, its close tree and the sha256 of its bytes; verification refuses a receipt whose latest closed station run is not PASS or whose bytes, close tree or path differ (receipt_matches_station_run).  A receipt written or edited by hand is not a receipt.
- Environment identity (section 5): a declared identity probe that fails or cannot be spawned identifies nothing; the receipt is FAIL (identity_probe_observed:<name>).
- Command confinement (section 5): a fixture command runs at "." or a literal relative directory of the workpiece and logs to a literal relative path (command_cwd_within_workpiece, command_log_within_workpiece); an escaping command is refused at station open.
- Canonical confinement (sections 5, 8): the canonical checkout (branch head and working-tree status) must be identical before and after a station's commands (canonical_repository_untouched_by_station) and before and after a re-inspection's commands (canonical_repository_untouched_by_reinspect); otherwise the receipt is FAIL, the re-inspection DIFFER.  With canonical_working_tree_clean and canonical_base_unmoved at integration, no path from an agent, a station or a probe to FactTest.git bypasses verification.
- Witness register: tests/factory/witnesses.json maps every refusal reason of the D21-D26 prompt to the Factory law tests (factory/tests/factory_law.rs f00-f28) and the text that refuses it; tests/factory/run-witnesses.sh runs and checks it and records the judge's identity; tests/sync/collision-witness.sh witnesses the parallel-line import collision.
